...
import com.zziafyc.base_library.R
 ...
 * 圆形图片
 */
fun ImageView.loadRadius(context: Context, url: String, radius: Float = 20.0F) {
    val into = Glide.with(context)
        .load(url)
        .centerCrop()
        .error(R.drawable.ic_launcher)
        .transition(withCrossFade())
        .transform(GlideRoundTransform(context, radius, radius, radius, radius))
        .into(this)
}
